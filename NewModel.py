...
# 定义模型
class NewModel():
    # def __init__(self, csa, ca, train_cour_num, behavior_num, lr=1e-3, device='cpu'):
    ...
    def fit(self, index_loader, train_data, test_data,
        ...
        for epoch in range(epochs):
            ...
            for batch_data in tqdm(index_loader, "[Epoch:%s]" % epoch):
                # 学生列表，每次从学生列表里选取一个batch的学生

                stu_list = np.array([x.numpy() for x in batch_data], dtype='int').reshape(-1)
                # stu_list = np.array([x.numpy() for x in batch_data], dtype='int')

                # 读取训练集
                train_pred, label_data = format_data(train_data.loc[stu_list, :])

                # print(f'train {np.array(train).shape} label {np.array(label_).shape}')

                # -----start training-------------------
                # train [[课程id]，[活跃度]] train[0]代表cour_list, train[1]代表acti_list
                # all_pred保存的是k折交叉验证后的数据。比如有10条数据，需要做5折，则需要10*5=50条数据,
                # 这里是每一次交叉验证后的预测结果
                all_pred = self.net(train_pred[0], train_pred[1], train_pred[2], save_a=False)

                # pred是从最后的输出结果里抽出来的那部分作预测loss的预测数据，每次追加是在上面的format_data实现的
                ...
                # 这里追加的数据是为了后面计算mse等来评估模型，这里应该是训练的预测值
            ...
            # pred_list[0]训练集的预测，label_list[0]训练集的真值 这里是求出训练集的rmse和mae
